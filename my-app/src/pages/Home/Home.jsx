import React from 'react'
import Navbar from './Navbar'
import ImageCarousel from './ImageCarousel';
import TravelList from './TravelList';
import { Link } from 'react-router-dom'
import './navbar.scss'
import { Navigate } from 'react-router-dom';
import './Home.scss'
const Home = () => {
  let curyear=new Date().getFullYear();
  return (
    <div className="home">
      <div className='navbar'>
      <div className="header">
          <div className='left'> 
           <span>Globetrot</span><span> &#9992;</span>
            </div>
            <div className='center'>
               <div>Destinations</div>
               <Link to='/review' style={{"textDecoration":"none"}}><div>Reviews</div></Link>
               <div>Packages</div>
               <div>Contact</div>
               <div>About</div>

            </div>
            <div className='right'>
              <button className='expl'>Explore</button>
          <Link to='/login'><button className='log'>Login</button></Link>
            <Link to='/register'><button className='sig'>SignUp</button></Link>
            </div>
         <div>

         </div>
        
        </div>
      </div>
      <div className='actual'>
      <div className='images'>
       <ImageCarousel img1="https://mir-s3-cdn-cf.behance.net/project_modules/1400_opt_1/823c7890819765.5e3bb868c7b09.jpg" img2="https://mir-s3-cdn-cf.behance.net/projects/max_808/35db94130641999.Y3JvcCwxMTkyLDkzMyw3MCww.jpg" /> 
      </div>
      <div className='aftercarousel'>
      <div className='topmost'>
      <div className='top'> Explore places cultures,
      </div>
      <div className='top1'>
         and many more
      </div>
      <div className='top2'>
        starts at &#8377;50000 cancel any time.
      </div>
      </div>
      <div className='topnext'>
        <div className='ton'>
           ready to travel ? Enter your email to start your journey
        </div>
        <div className='ton1'>
          <input type="text" placeholder="Enter email.."/>&nbsp;<Link to='/register'><button>Get Started</button></Link>
        </div>
      </div>
      
      </div>
      
       
      </div>
      <div className="after-carousel">
      <div className='imgs'>
       <div className='trav'>
        <h1> Are you |</h1>
        <div className='rea'>Ready To Travel <b style={{"color":"rgb(255, 121, 37)","font-size":"50px"}}>?</b></div>
       </div>
       </div>
        <div className='next'>
       <div className="box">
       <div className='adv'>
        <div>
          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR-_POxkEOFb43tY6Am9OKWysF3KadV4-0J6A&s"/>
        </div>
       <div className='ptg'><span>200+</span><br/> places</div>
       
       </div>
    
       <div className='adv'>
        <div>
        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQczRZiGjn1VtKHpmAEXm1ddzIflI8UuZnoWQ&s"/>
        </div>
       <div className='ptg'><span>100+</span><br/> tours</div>
       
       </div>
       <div className="adv">
        <div>
        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRb8cuPw-e6FQPoX85rKhYTofSiWPHuHGYQtQ&s"/>
        </div>
       <div className='ptg'><span>60+</span><br/> Expect Guides</div>
       </div>

       </div>
       </div>
    
      </div>

    <div>

    </div>
    <div className='city-cards'>
      <h1>Explore the places</h1>
    <TravelList/>
    </div>
    <hr/>
      <div className='footer2'>
        <div className='footer21' >
        <div className='use'>
        useful Information
      </div >
          <div>Having Issues</div>
          <div>Contact Information</div>
          <div>Travel Safety</div>
          <div>Make a Plan</div>
          <div>About us</div>
        </div>
        <div className='footer21'>
        <div className='use'>
        trekking
      </div>
         <div>Trekking Details</div>
         <div>Acommodations</div>
        </div>
        <div className='footer21'>
        <div className='use'>
        Accomodation
      </div>
         <div>find Guides</div>
         <div>Health Issues
         </div>
         <div>Requirements</div>
        </div>
        <div className='footer21'>
        <div className='use'>
        Pay With Us
      </div>
          <div className='paym'>
          The payment is encrypted and <br/>transmitted securely <br/>with an SSL protocol.
          </div>
          <div>
            <img src="data:image/png;base64,"/>
          </div>
          <div>
            Safety Pay
          </div>

        </div>
      </div>
      <div style={{"text-align":"center"}}>
        Copyright&copy; {curyear} Globetrot
      </div>
    
    </div>
  )
}

export default Home
